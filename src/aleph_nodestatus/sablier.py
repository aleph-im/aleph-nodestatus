...
def get_contract_abi():
    return json.load(
        open(os.path.join(Path(__file__).resolve().parent, "abi/Sablier.json"))
    )
...
async def process_contract_history(
    contract_address, start_height, balances=None, streams=None, last_seen=None
):
    web3 = get_web3()
    ...
    async for i in get_logs(web3, contract, start_height):
        ...
        if (
            evt_data["event"] == "CreateStream"
            and args["tokenAddress"] == settings.ethereum_token_contract
        ):
            balances[args["recipient"]] = (
                balances.get(args["recipient"], 0) + args["deposit"]
            )
            changed_addresses.add(args["recipient"])
            streams[args["streamId"]] = {"balance": args["deposit"], **args}

        elif evt_data["event"] == "CancelStream" and args["streamId"] in streams:
            balances[args["recipient"]] -= streams[args["streamId"]]["balance"]
            changed_addresses.add(args["recipient"])
            del streams[args["streamId"]]

        elif evt_data["event"] == "WithdrawFromStream" and args["streamId"] in streams:
...
async def sablier_monitoring_process():
    ...
    items = process_contract_history(
        settings.ethereum_sablier_contract,
        settings.ethereum_sablier_min_height,
